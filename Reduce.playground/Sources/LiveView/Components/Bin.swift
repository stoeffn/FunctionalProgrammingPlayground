...
final class Bin: Composable {
    ...
    private lazy var binNode: SKSpriteNode = {
        let node = SKSpriteNode(texture: filterCasingTexture)
        node.centerRect = CGRect(x: 0.4, y: 0, width: 0.2, height: 1)
        node.xScale = (self.size.width + 32) / filterCasingTexture.size().width
        node.yScale = self.size.height / filterCasingTexture.size().height
        return node
    }()
}
